      ...
      <main className="flex-grow">
        ...
        <section className={`py-20 text-center ${lightBg}`}>
          <div className="max-w-4xl mx-auto px-4">
            <h1 className={`text-5xl font-extrabold mb-3 ${primaryHighlight}`}>How Transparency Works</h1>
            <p className={`text-xl ${isDark ? "text-zinc-400" : "text-zinc-600"}`}>
              Follow your donation's journey, step-by-step, from your wallet to global impact.
            </p>
          </div>
        </section>
        ...
        <section className={`py-16 ${darkBg}`}>
          <div className="max-w-7xl mx-auto px-6">
            <h2 className={`text-3xl font-bold text-center mb-12 ${isDark ? "text-white" : "text-zinc-800"}`}>
              The Traceable Donation Process
            </h2>

            <div className="space-y-12">
              {steps.map((step, index) => (
                <div key={index} className="flex flex-col md:flex-row items-start md:space-x-8">
                  {/* Icon & Step Number */}
                  <div className="flex-shrink-0 mb-4 md:mb-0">
                    <div
                      className={`w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold border-4 ${stepColor} ${
                        isDark ? "bg-zinc-800 border-green-500" : "bg-gray-100 border-green-600"
                      }`}
                    >
                      0{index + 1}
                    </div>
                  ...
                  <div className="flex-grow">
                    <h3 className={`text-2xl font-extrabold mb-2 ${stepColor}`}>{step.title}</h3>
                    <p className={`text-lg mb-3 ${isDark ? "text-zinc-200" : "text-zinc-700"}`}>{step.description}</p>
                    <p className={`italic ${isDark ? "text-zinc-400" : "text-zinc-500"}`}>{step.detail}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className={`py-16 ${lightBg}`}>
          <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className={`mb-4 text-lg ${isDark ? "text-zinc-300" : "text-zinc-700"}`}>
                We don't rely on human intervention to approve fund releases. Instead, we use Smart Contracts
                —self-executing agreements coded directly onto the blockchain.
              </p>
              <ul className="list-disc list-inside space-y-2 ml-4">
                <li>
                  Automatic Release: Funds are released only when digital proof of work (e.g., photo verification,
                  signed document hash) is submitted and verified.
                </li>
                <li>Zero Delay: Eliminates bureaucratic holdups common in traditional aid distribution.</li>
                <li>Code is Law: The contract ensures funds are used exactly as designated by the donor.</li>
              </ul>
            </div>
            <div
              className={`p-6 rounded-lg ${
                isDark ? "bg-zinc-900" : "bg-blue-50"
              } border border-solid border-opacity-30 ${isDark ? "border-blue-700" : "border-blue-300"}`}
            >
              {/* 

[Image of a smart contract flow chart]
 */}
              <p className={`italic text-center text-xl ${isDark ? "text-zinc-400" : "text-zinc-500"}`}>
        ...
        <section className={`py-16 text-center ${darkBg}`}>
          <h2 className={`text-4xl font-bold mb-4 ${primaryHighlight}`}>Ready to Start Tracking?</h2>
          <p className={`text-xl mb-8 max-w-2xl mx-auto ${isDark ? "text-zinc-300" : "text-zinc-700"}`}>
            Witness the future of verifiable giving by exploring our live, public ledger today.
          </p>
          <a
